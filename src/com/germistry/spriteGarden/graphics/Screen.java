...
public class Screen {
	...
	public void renderMob16(int xpixel, int ypixel, Mob mob) {
		xpixel -= xOffset;
		ypixel -= yOffset;
		for(int y = 0; y < 16; y++) {
			int yabs = y + ypixel;
			...
			for(int x = 0; x < 16; x++) {
				int xabs = x + xpixel;
				int xsprite = x;
				if(xabs < -width || xabs >= width || yabs < 0 || yabs >= height) break;
				if(xabs < 0) xabs = 0;
				int col = mob.getSprite().pixels[xsprite + ysprite * 16];
				if (col != ALPHA_COL) pixels[xabs + yabs * width] = col;
			}
		}
	}
	public void renderMob32(int xpixel, int ypixel, Mob mob) {
		xpixel -= xOffset;
		ypixel -= yOffset;
		for(int y = 0; y < 32; y++) {
			int yabs = y + ypixel;
			...
			for(int x = 0; x < 32; x++) {
				...
				if(xabs < -width || xabs >= width || yabs < 0 || yabs >= height) break;
				if(xabs < 0) xabs = 0;
				int col = mob.getSprite().pixels[xsprite + ysprite * 32];
				if (col != ALPHA_COL) pixels[xabs + yabs * width] = col;
			}
		}
	}
	public void renderMob48(int xpixel, int ypixel, Mob mob) {
		xpixel -= xOffset;
		ypixel -= yOffset;
		for(int y = 0; y < 48; y++) {
			int yabs = y + ypixel;
			...
			for(int x = 0; x < 48; x++) {
				...
				if(xabs < -width || xabs >= width || yabs < 0 || yabs >= height) break;
				if(xabs < 0) xabs = 0;
				int col = mob.getSprite().pixels[xsprite + ysprite * 48];
				if (col != ALPHA_COL) pixels[xabs + yabs * width] = col;
			}
		}
	}
	public void renderMob64(int xpixel, int ypixel, Mob mob) {
		xpixel -= xOffset;
		ypixel -= yOffset;
		for(int y = 0; y < 64; y++) {
			int yabs = y + ypixel;
			...
			for(int x = 0; x < 64; x++) {
				...
				if(xabs < -width || xabs >= width || yabs < 0 || yabs >= height) break;
				if(xabs < 0) xabs = 0;
				int col = mob.getSprite().pixels[xsprite + ysprite * 64];
				if (col != ALPHA_COL) pixels[xabs + yabs * width] = col;
			}
		}
	}
